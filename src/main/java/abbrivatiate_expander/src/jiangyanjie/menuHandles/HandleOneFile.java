...
public class HandleOneFile {
    ...
    private void handleMethodInvocation(MethodInvocationInfo methodInvocationInfo) {
        ...
        Utils.putHashSet(AllExpansions.idToFiles, methodInvocationInfo.methodName.id, LX.tempFile);
        AllExpansions.idToMethodName.put(methodInvocationInfo.methodName.id, methodInvocationInfo.methodName);

        Utils.putHashSet(AllExpansions.idToFiles, methodInvocationInfo.methodName.type.id, LX.tempFile);
        ...
        for (Argument argument : arguments) {
            ...
            Utils.putHashSet(AllExpansions.idToFiles, type.id, LX.tempFile);
            AllExpansions.idToClassName.put(type.id, type);

            ArrayList<Identifier> identifiers = argument.identifiers;

            for (Identifier identifier : identifiers) {
                Utils.putHashSet(AllExpansions.idToFiles, identifier.id, LX.tempFile);
                ...
                if (identifier instanceof Parameter) {
                    ...
                    Utils.putHashSet(AllExpansions.idToFiles, parameter.type.id, LX.tempFile);
    ...
    private void handleMethodDeclaration(MethodDeclarationInfo methodDeclarationInfo) {
        ...
        Utils.putHashSet(AllExpansions.idToFiles, methodDeclarationInfo.methodName.id, LX.tempFile);
        AllExpansions.idToMethodName.put(methodDeclarationInfo.methodName.id, methodDeclarationInfo.methodName);

        if (methodDeclarationInfo.methodName.type != null) {
            Utils.putHashSet(AllExpansions.idToFiles, methodDeclarationInfo.methodName.type.id, LX.tempFile);
        ...
        for (Parameter parameter1 : parameters) {
            Utils.putHashSet(AllExpansions.idToFiles, parameter1.id, LX.tempFile);
            AllExpansions.idToParameter.put(parameter1.id, parameter1);

            Utils.putHashSet(AllExpansions.idToFiles, parameter1.type.id, LX.tempFile);
    ...
    private void handleExtend(ClassInfo classInfo) {
        ...
        Utils.putHashSet(AllExpansions.idToFiles, className.id, LX.tempFile);
        AllExpansions.idToClassName.put(className.id, className);

        ArrayList<ClassName> classNames = classInfo.expans;
        for (ClassName className1 : classNames) {
            Utils.putHashSet(AllExpansions.idToFiles, className1.id, LX.tempFile);
        ...
        for (Field field : fields) {
            Utils.putHashSet(AllExpansions.idToFiles, field.id, LX.tempFile);
            AllExpansions.idToField.put(field.id, field);

            Utils.putHashSet(AllExpansions.idToFiles, field.type.id, LX.tempFile);
        ...
        for (MethodName methodName : methodNames) {
            Utils.putHashSet(AllExpansions.idToFiles, methodName.id, LX.tempFile);
            AllExpansions.idToMethodName.put(methodName.id, methodName);

            if (methodName.type != null) {
                Utils.putHashSet(AllExpansions.idToFiles, methodName.type.id, LX.tempFile);

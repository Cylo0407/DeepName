package abbrivatiate_expander.src.Wiki;
...
import abbrivatiate_expander.src.Step2.Heu;
...
//		Wiki("calc");
    ...
    private static String Parse(String rawStr) {// extract rawStr
        String raw = rawStr;
        if (raw.contains("<")) {
    ...
    private static boolean CheckOk(String item, String Possi) {
        ...
        System.out.println(Heu.H1(item, Possi) || Heu.H2(item, Possi) || Heu.H3(item, Possi));

        return (Heu.H1(item, Possi) || Heu.H2(item, Possi) || Heu.H3(item, Possi));
//		return false;
    }
...
//			Document doc = Jsoup.parse(new File("E:/materials/test.html"),"utf8");
        ...
        Document doc = null;
        try {
            doc = Jsoup.parse(new URL("https://en.wikipedia.ahau.cf/wiki/" + item), 3000);
        } catch (IOException e) {
            try {
                doc = Jsoup.parse(new URL("https://en.wikipedia.ahau.cf/wiki/index.php?search=" + item), 3000);
